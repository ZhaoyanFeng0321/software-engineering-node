...
export default class FollowController implements FollowControllerI {
    ...
    public static getInstance = (app: Express): FollowController => {
        if(FollowController.followController === null) {
            ...
            app.delete("/api/users/:uid/followedby/:xuid", FollowController.followController.userRemoveFollower);
        }
    ...
    findAllFollows = (req: Request, res: Response) =>
        FollowController.followDao.findAllFollows()
            .then((follows: Follows[]) => res.json(follows));

    /**
     * @param {Request} req Represents request from client, including the
     * path parameters uid and xuid representing the user that is followed
     * another user and the user being removed from followers of a user
     * @param {Response} res Represents response to client, including status
     * on whether deleting the follow was successful or not
     */
    userRemoveFollower = (req: Request, res: Response) =>
        FollowController.followDao.userRemoveFollower(req.params.uid, req.params.xuid)
            .then(status => res.send(status));
};